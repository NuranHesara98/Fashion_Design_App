...
transporter.verify(function(error, success) {
  if (error) {
    ...
    console.error('Email settings:', {
      ...
      pass: process.env.EMAIL_APP_PASSWORD ? '****' + process.env.EMAIL_APP_PASSWORD.substring(process.env.EMAIL_APP_PASSWORD.length - 4) : 'Not set'
    });
  } else {
...
const sendWelcomeEmail = async (email: string, name: string): Promise<boolean> => {
  try {
    ...
    console.log(`Using email configuration: ${process.env.EMAIL_USER}, Password: ${process.env.EMAIL_APP_PASSWORD ? '****' + process.env.EMAIL_APP_PASSWORD.substring(process.env.EMAIL_APP_PASSWORD.length - 4) : 'Not set'}`);
    ...
    // Create a dedicated transporter for this email to ensure fresh connection
    const dedicatedTransporter = nodemailer.createTransport({
      service: 'gmail',
      host: 'smtp.gmail.com',
      port: 587,
      secure: false,
    ...
    await dedicatedTransporter.verify();
    console.log('Transporter verified successfully');

    const info = await dedicatedTransporter.sendMail(mailOptions);
...
export const register = async (
  ...
  try {
    ...
    try {
      ...
      console.log(`Welcome email sending status: ${emailSent ? 'Success' : 'Failed'}`);
      
      // Even if email fails, we continue with registration
      // but we log the issue for troubleshooting
      if (!emailSent) {
